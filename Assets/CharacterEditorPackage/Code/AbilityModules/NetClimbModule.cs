...
public class NetClimbModule : GroundedControllerAbilityModule
{
    ...
    [SerializeField] private MMF_Player onClimb;

    [SerializeField] float secondDelay = 1.25f;
    ...
    public override bool IsApplicable()
    {
        ...
        if (Net)
        {
            if (!m_IsActive)
            { 
                if (GetDirInput("Move").m_Direction != DirectionInput.Direction.Neutral)
                { 
                    Vector3 newPoint = Net.GetClosestPointOnNet(m_ControlledCollider.GetCapsuleTransform().GetPosition());
